...
of meshes with different step $h$ was compared either against an analytical solution of the full model 
or against the reference solution of the full model on a sufficiently fine mesh. 
This approach allows to distinguish numerical error and the error of the reduced model.
...
We tested a problem with fixed constant conductivity $\tn A=\tn I$ and with the exact solution $p^\delta(x,y)=e^x\cos y$ on the domain $\Omega:=(-1,1)\times(0,1)$
...
$\norm{\prtl^2_x p}_{L^\infty(\Omega_f)}$, which stands on the right hand side of \eqref{eq:error_estimates_delta}, is close to $1$ in this case.
Unfortunately, the numerical error of the velocity on the matrix domain is only of the first order which leads to worse numerical estimate of the order of convergence with 
...
In the second test investigated more realistic case, with $K_m=1$ and $K_f=100$. We prescribe harmonic Dirichlet boundary condition 
$p = {\mathrm cos}(x){\mathrm cosh}(y)$ on the boundary of the domain $\Omega:=(0,2)\times(-0.5,0.5)$ with the fracture along the line $x=1$.
Actually, we exploit symmetry of the problem, computing only on the top half and prescribing the homogeneous 
Neumann condition on the bottom boundary. In this case, no analytical solution is available, so we solve the full model on a mesh locally refined 
around fracture and especially around its endpoint $[1, 0.5]$, where the solution exhibits singularities. The continuum-fracture model is solved 
...
The error on the matrix domain converge with order $1$ only and the error on the fracture domain stagnates. Moreover 
the error of the velocity is quite large. The main reason of this situation is high and possibly unbounded norm $\norm{\prtl^2_x p}_{L^\infty(\Omega_f)}$,
which has been numerically estimated as $10^3$ and uniformly growing with decreasing $h$. Unfortunately, this scenario is common in applications. 
On the other hand even if the norm of second derivative is not bounded, we observe relatively good approximation of the pressure on both domains
and good approximation of the velocity on the matrix domain. The conclusion is that the solution of the full model usually contains singularities at their
intersection of the fracture boundary with the domain boundary which may lead to the inaccuracy in the continuum-fracture model. Refinement or dedicated model 
may be necessary in order to obtain descent flux error at the fracture boundary. 
