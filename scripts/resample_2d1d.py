...
# script extract from given dataset all elements of specified type (lines, triangles, tetrahedra).
...
# name of geometry dataset in the pipeline
SourceName="flow.pvd"
TOL = 1e-5
...
def resample_to_2d_1d(pdi, pdo, geom):
    ...
    barycenters_2d[:,0]=np.where(barycenters_2d[:,0]<0, barycenters_2d[:,0], barycenters_2d[:,0]+X_SHIFT)
    ...
    y_points=np.array((0))
    # reference grid
    x=np.linspace(0,X_SHIFT,AVERAGE_POINTS)
