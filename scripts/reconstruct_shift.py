...
def reconstruct_from_1d(pdi, pdo, x_shift):
    ...
    for i_old in range(n_points_old):
        if (points_mask[i_old]==0) :
            ...
            # duplicate
            point_a=point_b=point_c=pdi.GetPoint(i_old)
            point_b =(point_a[0]+x_shift, point_a[1], point_a[2])
            point_c =(point_a[0]+x_shift*0.5, point_a[1], point_a[2])
            new_id_a=new_points.InsertNextPoint(point_a)
            new_id_b=new_points.InsertNextPoint(point_b)
            new_id_c=new_points.InsertNextPoint(point_c)
            point_new_id[i_old]=[new_id_a, new_id_b, new_id_c]                        
    ...
    for i_old in range(n_cells_old) :
        ...
        if (cell_mask[i_old]==2) :
            # QUAD from LINE
            new_cell_point_ids.append(VTK_LINE)
            points=[cell.GetPointId(0),cell.GetPointId(1)]
            new_cell_point_ids.append(point_new_id[ points[0] ][2])
            new_cell_point_ids.append(point_new_id[ points[1] ][2])
        else :
            # renumber points
    ...
    for i in range(pdi.GetPointData().GetNumberOfArrays()) :
        ...
        for i_old in range( n_points_old ) :
            if (points_mask[i_old]==2) :
                ...
                newArray.SetTupleValue(i_new[2], oldArray.GetTuple(i_old) )
            else:    
                i_new = point_new_id[ i_old ]  
